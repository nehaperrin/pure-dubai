...
def get_search_terms(query):
    ...
    if q in mappings: return mappings[q]
    for key in mappings:
        if key in q: return mappings[key] + [q]
    return [q]

# --- CSS (Nuclear Burgundy) ---
st.markdown("""
    <style>
...
banned_ingredients = []
for pack in active_filters: banned_ingredients.extend(FILTER_PACKS[pack])
...
with tab1:
    ...
    with col_s1:
        cat_select = st.selectbox("Category", ["All Categories", "Baby Snacking", "Lunchbox", "Dairy & Alt", "Pantry"], label_visibility="collapsed")
    with col_s2:
        ...
        if results.empty:
            ...
            for index, row in results.iterrows():
                ...
                for bad in banned_ingredients:
                    if bad.lower() in ing_list.lower():
                        if bad.lower() == "salt":
                            ...
                            continue 
                        elif bad in FILTER_PACKS["Salt Watch"]:
                             if salt_g > 1.5: found_dangers.append(f"High Salt ({salt_g}g)")
                             else: warnings.append(f"Contains Salt ({salt_g}g)")
                        else:
                ...
                if "High Natural Sugars (>15g)" in active_filters:
                    ...
                    if any(k in ing_list.lower() for k in natural_keywords) and sugar_g > 15:
                ...
                with st.container():
                    ...
                    with col_info:
                        if is_safe:
                            if warnings: st.markdown('<span class="warning-tag">⚠️ CHECK LABEL</span>', unsafe_allow_html=True)
                            else: st.markdown('<span class="safe-tag">✓ VERIFIED SAFE</span>', unsafe_allow_html=True)
                        else:
                        ...
                        if is_safe and warnings: st.caption(f"Trace: {', '.join(warnings)}")
                        elif not is_safe: st.markdown(f":red[{', '.join(found_dangers)}]")
                        with st.expander("Ingredients"): st.caption(ing_list)
                    with col_action:
                        st.write("")
                        if is_safe:
...
# --- TAB 2, 3, 4, 5 (Condensed) ---
with tab2:
    st.markdown("### Research Journal")
    st.markdown('<div class="knowledge-card"><div class="knowledge-title">The Nitrate Problem</div><div class="knowledge-body">Commonly found in processed meats, Nitrates are linked to cell damage. We scan for Sodium Nitrite and Potassium Nitrate automatically.</div></div>', unsafe_allow_html=True)
    st.markdown('<div class="knowledge-card"><div class="knowledge-title">Red 40 & Hyperactivity</div><div class="knowledge-body">The "Southampton Six" colours (including Red 40) increase hyperactivity in children. The EU requires a warning label; we just ban them.</div></div>', unsafe_allow_html=True)

with tab3:
    st.markdown("### The SIFT Method")
    st.caption("1. SELECT PROFILE | 2. SCAN PRODUCT | 3. DECIDE")
    st.markdown('<div class="earthy-green"><b>✓ VERIFIED SAFE</b></div><br><div class="earthy-yellow"><b>⚠️ CHECK LABEL</b></div><br><div class="earthy-red"><b>✕ AVOID</b></div>', unsafe_allow_html=True)
...
with tab4:
    ...
    if not st.session_state['wishlist']: st.markdown('<div class="charcoal-alert">No favourites yet.</div>', unsafe_allow_html=True)
    else:
        for item in st.session_state['wishlist']: st.markdown(f"**{item['Product']}**"); st.divider()
...
with tab5:
    ...
    if not st.session_state['basket']: st.markdown('<div class="charcoal-alert">Your basket is empty.</div>', unsafe_allow_html=True)
    else:
        for item in st.session_state['basket']: st.markdown(f"**{item['Product']}**")
        st.divider()
...
with tab6:
    ...
    with col_d1:
        st.markdown("**Top Searched (Last 7 Days)**")
        mock_search = pd.DataFrame({'Term': ['Nitrate-Free Ham', 'Dairy Free Yoghurt', 'Nut Free Bar', 'Oat Milk Kids'], 'Volume': [850, 620, 450, 300]})
        st.bar_chart(mock_search.set_index('Term'))
    
    with col_d2:
        st.markdown("**Most 'Avoided' Ingredients**")
        mock_avoid = pd.DataFrame({'Ingredient': ['Sodium Nitrite', 'Red 40', 'High Fructose Syrup', 'Palm Oil'], 'Rejections': [1200, 980, 850, 600]})
        st.bar_chart(mock_avoid.set_index('Ingredient'), color='#FF4B4B')
