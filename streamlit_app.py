...
# --- SMART SEARCH ENGINE (The "Brain") ---
def get_search_terms(query):
    """
    Converts user input (e.g. 'yaourt') into database-friendly terms.
    """
    q = query.lower().strip()
    
    # The Rosetta Stone Dictionary
    mappings = {
        # YOGHURT (Includes all spellings + brands)
        ...
        "yaourt": ["yog", "petit filous", "yeo", "nabta", "frubes", "actimel"],
        
        # SNACKS (Bars, Chips, Wafers)
        "snack": ["chip", "crisp", "puff", "bar", "wafer", "biscuit", "cookie"],
        "snacks": ["chip", "crisp", "puff", "bar", "wafer", "biscuit", "cookie"],
        "chips": ["crisp", "chip", "puff", "popcorn"],
        "crisps": ["crisp", "chip", "puff", "popcorn"],
        
        # MILK (Strict - No Yoghurt)
        "milk": ["milk", "soya", "oat", "koita", "alpro", "drink"],
        
        # CHEESE
        "cheese": ["cheese", "cheestrings", "kiri", "babybel", "spread"],
        
        # FRUIT
        "fruit": ["fruit", "raisin", "mango", "apple", "bear", "yo yos", "paws", "strawberry"],
    ...
    # 2. Partial Match? (e.g. "strawberry yoghurt")
    for key in mappings:
        if key in q:
            return mappings[key] + [q] # Search for category + specific word
            
    # 3. No match? Search exactly what they typed.
    return [q]
...
st.markdown("""
    ...
    html, body, [class*="css"] { font-family: 'Montserrat', sans-serif; color: #333333; }
    .stApp { background-color: #F9F9F7; }

    /* RED KILLER: Override Streamlit Defaults */
    input:focus, textarea:focus, select:focus, div[data-baseweb="select"]:focus-within, div[data-baseweb="input"]:focus-within { border-color: #5D0E1D !important; box-shadow: 0 0 0 1px #5D0E1D !important; }
...
if 'active_profile' not in st.session_state: st.session_state['active_profile'] = list(st.session_state['profiles'].keys())[0]
...
# NOTE: Typo fixed 'Youghurt' -> 'Yoghurt' in Bright Bites
kids_db = [
                ...
                for bad in banned_ingredients:
                    if bad.lower() in ing_list.lower():
                        if bad.lower() == "salt":
                            ...
                            continue 
                        elif bad in FILTER_PACKS["Salt Watch"]: # Salt Logic Handle
                             if salt_g > 1.5: found_dangers.append(f"High Salt ({salt_g}g)")
                             else: warnings.append(f"Contains Salt ({salt_g}g)")
                        else:
...
# --- TEMPORARY BUSINESS PLAN GENERATOR ---
# Paste this at the VERY BOTTOM of streamlit_app.py
# After downloading the file, you can delete this block.
...
try:
    ...
    def create_presentation():
        ...
        slide.shapes.title.text = "SIFT KIDS"
        slide.placeholders[1].text = "The Digital Food Guard for Modern Mums\nPitch Deck"

        # SLIDE 2: The Problem
        add_slide("The Problem: 'The Yoghurt Aisle Crime Scene'", [
            "Insight: Mums suffer from 'Label Fatigue'.",
            "Pain Point: It takes a Chemistry degree to understand ingredients.",
            "Stat: Average mum spends 4 mins per aisle reading labels.",
            "Result: Anxiety, wasted money on 'fake healthy' food, and health risks (allergies, hyperactivity)."
        ])

        # SLIDE 3: The Solution
        add_slide("The Solution: SIFT KIDS", [
            "What it is: A 'Zero-Inference' search engine.",
            "Mechanism: Binary 'Safe' or 'Avoid' verdict based on strict filters.",
            "User Flow:",
            "  1. Filter (e.g., 'Max - Nut Free').",
            "  2. Scan (Search 'Yoghurt').",
            "  3. Result (Green for Safe, Red for Avoid)."
        ])

        # SLIDE 4: The Strategy (The Wedge)",
        add_slide("Strategy: The 'Wedge' Approach", [
            "Why Kids First?",
            "  Higher Stakes: Mums never compromise on baby food.",
            "  Higher Frequency: Kids snack 3-5 times a day.",
            "  Viral Growth: WhatsApp groups & School Gates are free distribution.",
            "Target Market: Dubai’s ingredient-conscious expat mum community."
        ])

        # SLIDE 5: Revenue Model 1 (Affiliate)
        add_slide("Phase 1: The Affiliate Play (Immediate)", [
            "Logic: We don't hold inventory. We are a High-Intent Funnel.",
            "Mechanism: User clicks 'Buy at Kibsons' via SIFT.",
            "Revenue: 5-10% commission on basket checkout.",
            "Value: Solves the 'Leakage' problem by integrating directly into retailer carts."
        ])

        # SLIDE 6: Revenue Model 2 (Data)
        add_slide("Phase 2: The Data Play (Mid-Term)", [
            "Logic: Retailers know what people buy. SIFT knows what they wanted but rejected.",
            "Product: 'Intent Data' Reports.",
            "  Example: '800 Mums rejected your Turkey slices due to Nitrates.'",
            "Revenue: B2B Subscriptions for Retailers (AED 5k-10k/mo)."
        ])

        # SLIDE 7: Revenue Model 3 (Trust Mark)
        add_slide("Phase 3: The Trust Mark (Long-Term)", [
            "Logic: SIFT becomes the gold standard for safety.",
            "Product: The 'SIFT VERIFIED' Seal licensing.",
            "Revenue: Annual licensing fees for brands to print logo on packaging.",
            "Comparable: Clean Label Project, AHA Heart Check."
        ])

        # SLIDE 8: Go-To-Market
        add_slide("Go-To-Market: The 'Whisper Launch'", [
            "Step 1: Alpha (Now): 'Kitchen Table' validation.",
            "Step 2: Beta (Month 1): Private WhatsApp groups (50 Mums).",
            "Step 3: Public (Month 3): 'Back to School' campaign."
        ])

        # SLIDE 9: Competition & Moat
        add_slide("Competition vs. Our Moat", [
            "Competitors: Yuka (US-centric), MyFitnessPal (Calorie focused).",
            "SIFT Moat:",
            "  1. Hyper-Local: Mapped to Dubai shelves (Kibsons/Spinneys).",
            "  2. Strict Logic: Bans chemicals (Nitrates/Red 40), not just calories.",
            "  3. Trust: Founder-led story (Chemistry background)."
        ])

        # SLIDE 10: The Ask
        add_slide("Current Status & The Ask", [
            "Status: Working Prototype with Real Database.",
            "Immediate Goal: Secure 50 Beta Testers.",
            "Next Milestone: Validate Affiliate Model with 100 click-throughs."
        ])
    ...
    st.write("Generate your Business Plan for Google Slides.")
    
    ppt_data = create_presentation()
    st.download_button(
        label="📥 DOWNLOAD BUSINESS PLAN (.pptx)",
        data=ppt_data,
        file_name="SIFT_KIDS_Business_Plan.pptx",
        mime="application/vnd.openxmlformats-officedocument.presentationml.presentation"
    )

except ImportError:
    st.divider()
    st.error("⚠️ To download the Business Plan, please install the library: pip install python-pptx")
